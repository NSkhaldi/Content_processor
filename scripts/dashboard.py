...
def update_output(value, interval):
    ...
    fig.add_trace(go.Pie(labels=list(connected_hosts.keys()), values=list(connected_hosts.values()), textinfo='label+value', name='connected to', hole=.65),
              row=1, col=1)
    fig.add_trace(go.Pie(labels=list(received_hosts.keys()), values=list(received_hosts.values()), textinfo='label+value', name='received from',  hole=.65),
              row=1, col=2)
    fig.add_trace(go.Bar(x=list(active_hosts.keys()), y=list(active_hosts.values()), name="All connections", marker=dict(color='orange', coloraxis="coloraxis") ),
              row=2, col=1)
